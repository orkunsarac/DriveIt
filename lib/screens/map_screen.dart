...
class MapScreen extends StatefulWidget {
  ...
  const MapScreen({super.key, this.resumeDrive = false, this.finishOnOpen = false});
...
class _MapScreenState extends State<MapScreen> {
  ...
  void _setStartMarker(LatLng position) {
    locationMarkers = {
      ...locationMarkers.where((marker) => marker.markerId.value != 'driveit_start'),
      Marker(
  ...
  void _setFinishMarker(List<RoutePoint> route) {
    ...
    final finish = Marker(
      ...
      icon: _finishIcon ?? BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueMagenta),
      anchor: const Offset(.5, .5),
      flat: true,
      infoWindow: const InfoWindow(title: 'Bitiş noktası'),
    );
    locationMarkers = {
      ...locationMarkers.where((marker) => marker.markerId.value != 'driveit_finish'),
      finish,
    };
  }
  ...
  Future<BitmapDescriptor> _createRaceEndpointIcon(
    ...
    canvas.drawCircle(center, 7, ui.Paint()..color = const ui.Color(0xff071426));
    canvas.drawCircle(
      center,
      7,
      ui.Paint()
  ...
  Future<bool> _handleBack() async {
    ...
    final action = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Sürüşün hâlâ kaydediliyor. Sürüşü bitirmeden bu ekrandan çıkamazsın.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, 'continue'), child: const Text('Sürüşe dön')),
          FilledButton(onPressed: () => Navigator.pop(context, 'finish'), child: const Text('Sürüşü bitir ve kaydet')),
        ],
      ),
    );
  ...
  Future<void> _createArrowIcon([double scale = 1]) async {
    ...
    final fill = ui.Paint()..shader = ui.Gradient.linear(const ui.Offset(0, 0), ui.Offset(size, size), [const ui.Color(0xffff4d6d), const ui.Color(0xffd50032)]);
    ...
    if (data != null && mounted) {
      final icon = BitmapDescriptor.bytes(Uint8List.view(data.buffer));
      setState(() {
        _arrowIcon = icon;
        if (_lastMarkerPosition != null) {
          locationMarkers = {
            ...locationMarkers.where((marker) => marker.markerId.value != 'driveit_current_location'),
            Marker(markerId: const MarkerId('driveit_current_location'), position: _lastMarkerPosition!, icon: icon, anchor: const Offset(.5, 1), flat: true, rotation: _markerRotation),
          };
        }
      });
    }
  ...
  void _updateLocationMarker(double latitude, double longitude, {double? heading}) {
    final next = LatLng(latitude, longitude);
    if (heading != null && heading.isFinite && heading >= 0 && heading < 360) {
      _markerRotation = heading;
    } else if (_lastMarkerPosition != null) {
      final distance = Geolocator.distanceBetween(_lastMarkerPosition!.latitude, _lastMarkerPosition!.longitude, latitude, longitude);
      if (distance > 1) {
        _markerRotation = Geolocator.bearingBetween(_lastMarkerPosition!.latitude, _lastMarkerPosition!.longitude, latitude, longitude);
      }
    }
    ...
    locationMarkers = {
      ...
      Marker(
        ...
        position: next,
        icon: _arrowIcon ?? BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRed),
  ...
  Future<void> getCurrentLocation() async {
    ...
    if (mounted) {
      setState(() => _updateLocationMarker(currentPosition!.latitude, currentPosition!.longitude));
    }
  ...
  Future<void> startDriving() async {
    ...
    positionStream =
        Geolocator.getPositionStream(
          ...
          setState(() {
            if (!_hasStartMarker && position.accuracy <= 30) {
              _setStartMarker(LatLng(position.latitude, position.longitude));
            }
            _updateLocationMarker(position.latitude, position.longitude, heading: position.heading);
  ...
  Future<void> stopDriving() async {
    ...
    final remainingBackgroundPoints = backgroundRoute.skip(_backgroundRouteIndex);
...
debugPrint(
  ...
  void initState() {
    ...
    if (widget.resumeDrive) {
      driveStartTime = await ForegroundService.readDriveStartTime() ?? DateTime.now();
      elapsedSeconds = DateTime.now().difference(driveStartTime!).inSeconds;
      elapsedTimer = Timer.periodic(const Duration(seconds: 1), (_) {
        if (!mounted || !isDriving || driveStartTime == null) return;
        setState(() => elapsedSeconds = DateTime.now().difference(driveStartTime!).inSeconds);
      });
      ...
      if (savedRoute.isNotEmpty && savedRoute.first['lat'] is num && savedRoute.first['lng'] is num) {
        _setStartMarker(LatLng((savedRoute.first['lat'] as num).toDouble(), (savedRoute.first['lng'] as num).toDouble()));
      }
    ...
    return WillPopScope(
      ...
      body: Stack(
        children: [
          ...
          SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 0), child: Row(children: [
            _control(Icons.arrow_back, () => Navigator.maybePop(context)),
            const SizedBox(width: 12),
            Expanded(child: Container(height: 58, decoration: BoxDecoration(color: const Color(0xee101c32), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white12)), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 20, height: 20, decoration: BoxDecoration(shape: BoxShape.circle, color: isDriving ? const Color(0xff48e8cb) : Colors.blueGrey.shade300)), const SizedBox(width: 10), const Flexible(child: FittedBox(fit: BoxFit.scaleDown, child: Text('SÜRÜŞ MERKEZİ', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)))), const SizedBox(width: 10), Text(_formatElapsed(), style: const TextStyle(color: Color(0xff4de3c8), fontSize: 17, fontWeight: FontWeight.w800))]))),
            const SizedBox(width: 12), _control(Icons.my_location, getCurrentLocation),
          ]))),
  ...
  Widget _control(IconData icon, VoidCallback onTap) => Material(color: const Color(0xee101c32), borderRadius: BorderRadius.circular(18), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(18), child: SizedBox(width: 58, height: 58, child: Icon(icon, color: Colors.white, size: 31))));
  Widget _drivePanel() => Container(padding: const EdgeInsets.fromLTRB(28, 22, 28, 22), decoration: BoxDecoration(color: const Color(0xf20e182b), borderRadius: BorderRadius.circular(28), border: Border.all(color: Colors.white12), boxShadow: const [BoxShadow(color: Color(0x55000000), blurRadius: 22)]), child: Column(mainAxisSize: MainAxisSize.min, children: [Row(children: [_stat('${currentSpeed.toStringAsFixed(0)}', 'KM/SA', const Color(0xff50e4cb)), _divider(), _stat((routeService.distance / 1000).toStringAsFixed(2), 'KM', Colors.white), _divider(), _stat(maxSpeed.toStringAsFixed(0), 'MAKS.', Colors.white)]), const SizedBox(height: 24), SizedBox(width: double.infinity, height: 62, child: ElevatedButton.icon(onPressed: isDriving ? stopDriving : startDriving, icon: Icon(isDriving ? Icons.stop : Icons.play_arrow, color: Colors.black, size: 26), label: Text(isDriving ? 'SÜRÜŞÜ BİTİR' : 'SÜRÜŞE BAŞLA', style: const TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.w900)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xff72a6ff), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(22)), elevation: 0))) ]));
  Widget _stat(String value, String label, Color color) => Expanded(child: Column(children: [Text(value, style: TextStyle(color: color, fontSize: 31, fontWeight: FontWeight.w800)), const SizedBox(height: 5), Text(label, style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 1.2))]));
  String _formatElapsed() => '${(elapsedSeconds ~/ 60).toString().padLeft(2, '0')}:${(elapsedSeconds % 60).toString().padLeft(2, '0')}';
